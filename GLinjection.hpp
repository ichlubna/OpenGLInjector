...
public:
    ...
    public:
        ...
        {
            return static_cast<int>(params[key]);
        }
    };
    ...
    GLInjection(
        ...
        : renderWidth(renderWidth),
          ...
          viewWidth(quiltViewWidth),
          viewHeight(quiltViewHeight),
          params{configFile}
    {
        quiltWidth = viewWidth * params.i("Cols");
        quiltHeight = viewHeight * params.i("Rows");
    ...
    void render()
    {
        ...
        glUniform1f(
            glGetUniformLocation(shaderProgram, "holoTilt"),
            params["Tilt"]
        );

        glUniform1f(
            glGetUniformLocation(shaderProgram, "holoPitch"),
            params["Pitch"]
        );

        glUniform1f(
            glGetUniformLocation(shaderProgram, "holoCenter"),
            params["Center"]
        );

        glUniform1f(
            glGetUniformLocation(shaderProgram, "holoViewPortionElement"),
            params["ViewPortionElement"]
        );

        glUniform1f(
            glGetUniformLocation(shaderProgram, "holoSubp"),
            params["Subp"]
        );

        glUniform1i(
            glGetUniformLocation(shaderProgram, "viewCount"),
            views()
        );

        glUniform1i(
            glGetUniformLocation(shaderProgram, "holoCols"),
            params["Cols"]
        );

        glUniform1i(
            glGetUniformLocation(shaderProgram, "holoRows"),
            params["Rows"]
        );

        glUniform1i(
            glGetUniformLocation(shaderProgram, "mode"),
            params["Quilt"]
        );
...
private:
    ...
    const char *fragmentShaderSource = R""""(
        ...
        vec2 texArr(vec3 uvz)
        {
            ...
            float x =
                (mod(z, holoCols) + uvz.x)
                / holoCols;

            float y =
                (floor(z / holoCols) + uvz.y)
                / holoRows;

            return vec2(x, y)
                * vec2(
                    holoViewPortionElement,
                    holoViewPortionElement
                );
        }
